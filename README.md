...
CareerMap 是一个可部署的职业路径规划 MVP。用户输入当前岗位、年限、城市、教育经历和工作经历后，系统按步骤生成路径地图、能力差距报告，以及与目标岗位匹配的在招职位。目标岗位可以留空；职业测评是页面底部的单独可选功能。
...
- 数据：MVP 阶段使用结构化 AI 生成器，职位数据为可替换 JSON 结构
- 已做：用户信息录入、多段教育经历、经历摘要、目标岗位选填、底部可选职业倾向测评、分步骤职业路径地图、近/中/远横向迁移、能力差距分析、职位聚合与筛选
- 职业测评：页面底部可选功能，输出类似 MBTI 的职业倾向类型码和岗位推荐，不强制影响路径规划
- 职位数据：支持配置授权 API/代理接口接入 Boss、猎聘、前程无忧/51job、LinkedIn；未配置时展示 AI 生成职位和平台搜索入口
- 纵向晋升：针对办公室主任等岗位加入总经办、总经理助理、董事会秘书、幕僚长、副总/总经理等分支判断
- 横向迁移：按近迁移、中迁移、远迁移展示跨行业/跨职能方向，每档 3-5 个岗位，附可迁移能力、需补技能和案例入口
